...
`HomePortrait.astro` displays `src/assets/people/trey-turner.jpg` above the home page name in a circular crop, scaling from 128px on phones to 160px on desktop at the default text size. A decorative CSS circle sits slightly down and left behind the photo, retaining the old site's translucent yellow gradient in both themes. Astro generates responsive WebP images with explicit dimensions and eager loading; the photo and backdrop require no JavaScript. The `sizes` hint mirrors the `clamp(8rem, 20vw, 10rem)` layout and its narrow-screen cap: viewport width minus 3rem of container padding and the frame's 2rem allowance, floored at zero. Keep this hint in sync if those spacing values change. Source widths extend to 960px for 200% browser-default text on 3× displays. Source-selection tests include 320px viewports and change Chromium's actual default font size, since a CSS-only root font override does not change the preference used to resolve font-relative lengths in `sizes`.
